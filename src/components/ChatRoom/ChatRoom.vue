    ...
    <el-row>
      <el-col :span="8">
        <el-card style="width: 100%; min-height: 300px; color: #333">
          ...
          <div style="padding: 10px 0" v-for="user in users" :key="user.userEmail">
            <span>{{ user.userEmail }}</span>
            <i class="el-icon-chat-dot-round" style="margin-left: 10px; font-size: 16px; cursor: pointer"
               @click="switchAccount(user.userEmail)"></i>
            <span style="font-size: 12px;color: limegreen; margin-left: 5px" v-if="user.userEmail === chatUser">chatting...</span>
          </div>
        </el-card>
      </el-col>
      <el-col :span="16">
